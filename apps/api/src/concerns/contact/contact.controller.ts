...
export async function getContact(
  ...
  await context.with(
    ...
    async () => {
      ...
      contact = await getContactByContactIdByUserId(contactId, req.user.id);
        ...
        async () => {
          ...
          if (contact.tasks.length > 0) {
            ...
            if (!contact.tasks.includes(task)) {
              ...
              createContactTaskSpan.addEvent("Task added to end of list");
            }
          } else {
            contact.tasks.push(task);
          ...
          await context.with(
            ...
            async () => {
              const createContactHistoryEventSpan = tracer.startSpan(
              ...
              const newHistoryEvent = await createContactHistoryEvent(
                req.user.id,
                contact.id,
                "TASK_CREATED",
              );
